...
c     
     ...
     &        ' (call to sfstart in restrt_hdf.f)'
      ...
      index = index + 1
      call read_integer_vector(sd_id,maxlv,index,'intratz',intrtz)
      index = index + 1
      call read_integer_vector(sd_id,maxlv,index,'intratt',intrtt)
      index = index + 1
      call read_integer_vector(sd_id,maxlv,index,'iregsz',iregsz)
      index = index + 1
      call read_integer_vector(sd_id,maxlv,index,'jregsz',jregsz)
      index = index + 1
      call read_integer_vector(sd_id,maxlv,index,'kregsz',kregsz)
      index = index + 1
...
c     # HDF: DFNT_FLOAT64
      ...
      index = index + 1
      call read_double_vector(sd_id,maxlv,index,'hzposs',hzposs)
      index = index + 1
...
c     # add new info. to spatial and counting arrays
   ...
   35 if (level .gt. mxnest) go to 45
      ...
      kregsz(level) = kregsz(level-1) * dble(intratz(level-1))
      level         = level + 1
      go to 35
   45 continue
c
