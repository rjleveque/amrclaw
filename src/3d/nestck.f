...
c
 ...
 101     format(' nestck: 1 pt. cluster at level ',i5,' still not',
     1     ' nested',/,'          pt. too close to boundary')
         write(outunit,104) badpts(1,npts),badpts(2,npts),badpts(3,npts)
         write(*,104)       badpts(1,npts),badpts(2,npts),badpts(3,npts)
 ...
 103  format(' bisecting cluster ',i5,' with ',i5,' pts. in nestck')
      ...
      sidemx = dmax1(       rnode(cornxhi,mnew)-rnode(cornxlo,mnew),
     1                      rnode(cornyhi,mnew)-rnode(cornylo,mnew))
      sidemx = dmax1(sidemx,rnode(cornzhi,mnew)-rnode(cornzlo,mnew))
      if     (rnode(cornxhi,mnew)-rnode(cornxlo,mnew) .ge. sidemx) then
           rmid  = (rnode(cornxhi,mnew) + rnode(cornxlo,mnew) ) / 2.
           rmid  = (node(ndihi,mnew) + node(ndilo,mnew) + 1 ) / 2.
           rmid  = rmid / lratiox
           idir  = 1
      elseif (rnode(cornyhi,mnew)-rnode(cornylo,mnew) .ge. sidemx) then
           rmid  = (rnode(cornyhi,mnew) + rnode(cornylo,mnew) ) / 2.
           rmid  = (node(ndjhi,mnew) + node(ndjlo,mnew) + 1) / 2.
           rmid  = rmid / lratioy
           idir  = 2
      else
           rmid  = (rnode(cornzhi,mnew) + rnode(cornzlo,mnew) ) / 2.
           rmid  = (node(ndkhi,mnew) + node(ndklo,mnew) + 1) / 2.
           rmid  = rmid / lratioz
           idir  = 3
      endif
c
